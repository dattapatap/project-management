...
<script>
    $(function() {
        ...
        function loadRenewalClients(preselect) {
            ...
            $.get("{{ route('csd.clients.active') }}",
                function(res) {
                    ...
                    if (preselect) {
                        $sel.val(String(preselect)).trigger('change');
                    }
                }).fail(function() {
        ...
        function loadAmcOptions(clientId) {
            ...
            $.get("{{ route('csd.renewals.amc-options') }}", {
                    client: clientId
                },
                function(res) {
        ...
        $('#renewalTitle').on('input', function() {
        ...
        $('#frmAdd').on('submit', function(e) {
            e.preventDefault();
            csdApi.post("{{ route('csd.renewals.store') }}", $(this).serialize(),
                function() {
        ...
        $(document).on('click', '.editRenewal', function() {
            csdApi.get('/csd/renewals/' + $(this).data('id'), function(r) {
                if (!r.success) {
                    if (r.message) alertify.error(r.message);
                    return;
                }
                var d = r.data;
                $('#editId').val(d.id);
        ...
        $('#btnSyncRenewals').on('click', function() {
            ...
            csdApi.post("{{ route('csd.renewals.sync') }}", {},
                function() {
                    table.ajax.reload();
