...
<script>
    $(function() {
        var table = $('#dataTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('csd.collections.data') }}",
            columns: [{
                data: 'DT_RowIndex',
                orderable: false
            }, {
        ...
        $(document).on('click', '.editCollection', function() {
            csdApi.get('/csd/collections/' + $(this).data('id'), function(r) {
                ...
                var d = r.data;
                $('#editId').val(d.id);
                $('#editAmount').val(d.amount_due);
                $('#editDueDate').val(d.due_date ? d.due_date.split('T')[0] : '');
                $('#editFollowup').val(d.followup_date ? d.followup_date.split('T')[0] : '');
                $('#editCommitmentDate').val(d.commitment_date ? d.commitment_date.split('T')[0] : '');
                $('#editCommitmentAmount').val(d.commitment_amount || '');
        ...
        $('#frmEdit').on('submit', function(e) {
            e.preventDefault();
            csdApi.put('/csd/collections/' + $('#editId').val(), $(this).serializeArray().reduce((o, i) => {
                o[i.name] = i.value;
                return o;
            }, {}), function() {
