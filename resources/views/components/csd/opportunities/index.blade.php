...
<script>
    $(function() {
        var table = $('#dataTable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{ route('csd.opportunities.data') }}",
            columns: [{
                data: 'DT_RowIndex',
                orderable: false
            }, {
        ...
        $('#frmAdd').on('submit', function(e) {
            e.preventDefault();
            csdApi.post("{{ route('csd.opportunities.store') }}", $(this).serialize(),
                function() {
        ...
        $(document).on('click', '.editOpportunity', function() {
            csdApi.get('/csd/opportunities/' + $(this).data('id'), function(r) {
                ...
                $('#editStatus').val(d.status === 'won_pending_assignment' ? 'won' : d.status);
                $('#editFollowupDate').val(d.followup_date ? d.followup_date.substring(0, 10) : '');
                if ($('#editAssignedTo').length) {
                    $('#editAssignedTo').val(d.assigned_to || '');
                }
                $('#mdlEdit').modal('show');
            });
        });
        $('#frmEdit').on('submit', function(e) {
            e.preventDefault();
            var id = $('#editOpportunityId').val();
            csdApi.put('/csd/opportunities/' + id, $(this).serializeArray().reduce((o, i) => {
                o[i.name] = i.value;
                return o;
            }, {}), function() {
        ...
        // Assign to NSD Click Handler
        $(document).on('click', '.btnAssignNsd', function() {
            var id = $(this).data('id');
            var title = $(this).data('title');
            var client = $(this).data('client');

            $('#assignOpportunityId').val(id);
            $('#assignOpportunityTitle').text(title);
            $('#assignClientName').text(client);
            $('#assignSalesRepId').val('');
            $('#mdlAssignNsd').modal('show');
        });

        // Assign to NSD Submit Form Handler
        $('#frmAssignNsd').on('submit', function(e) {
            e.preventDefault();
            var id = $('#assignOpportunityId').val();
            csdApi.post('/csd/opportunities/' + id + '/assign-nsd', $(this).serialize(), function(response) {
                csdApi.closeModal('#mdlAssignNsd', '#frmAssignNsd');
                table.ajax.reload();
            });
        });
    });
